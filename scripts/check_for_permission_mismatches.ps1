<#
.SYNOPSIS
    Checks for permission mismatches in a SharePoint library.
.DESCRIPTION
    This script compares the permissions of items in a SharePoint document library
    against the top-level library permissions. It only logs errors when items have
    different permissions than the library itself. Items inheriting permissions
    are considered correct and are not logged.
.PARAMETER LibraryName
    The name of the SharePoint document library to check.
.EXAMPLE
    .\check_for_permission_mismatches.ps1 -LibraryName "Documents"
    Checks all items in the "Documents" library for permission mismatches.
.NOTES
...
$logFile = Initialize-Logging -LogFileName "permission_mismatches" -LogFolder $logFolder
...
# Start processing
Write-LogEntry "=== SharePoint Permission Mismatch Check Started ===" "INFO" $logFile
...
try {
            ...
            if (-not $hasUniqueRoleAssignments) {
                ...
                if ($itemPermissionSignature -ne $listPermissionSignature) {
                    ...
                    if ($isTopLevel) {
                        ...
                        Write-LogEntry "  Nested item has unique permissions that differ from library" "ERROR" $logFile
                        Write-LogEntry "" "INFO" $logFile
                    }
                }
            }
    ...
    Write-LogEntry "=== Check Complete ===" "SUCCESS" $logFile
    ...
    Write-LogEntry "Nested items with mismatched permissions: $nestedMismatchCount" $(if ($nestedMismatchCount -gt 0) { "ERROR" } else { "SUCCESS" }) $logFile
